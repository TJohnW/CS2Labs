...
/*
 * $Id: Strategy.java,v 1.2 2014/05/11 23:25:04 p142-06b Exp $
 * 
 * $Log: Strategy.java,v $
 * Revision 1.2  2014/05/11 23:25:04  p142-06b
 * Updated docs and some strategy
 *
 * Revision 1.1  2014/05/07 23:14:06  p142-06b
 * commit
 *
...
public class Strategy {
	...
	/**
	 * Does the player have a winning winning move? 
	 * @param player_id the player to check
	 * @param game_instance game state
	 * @return bool
	 */
	public static boolean hasWinningWinningMove(int player_id, Game game_instance) {
		List<PlayerMove> moves = Validation.generateValidMoves(player_id, game_instance);
		for(PlayerMove mv: moves) {
			Game cp = new Game(game_instance);
			cp.update(mv);
			if(hasWinningMove(player_id, cp))
				return true;
		}
		return false;
	}
